        ...
        {events.map(event => {
          ...
          <div key={event.id} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col">
            <div className="relative h-48 overflow-hidden">
              <img src={event.image || '/api/placeholder/400/300'} alt={event.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
            ...
            <div className="p-6 flex-grow flex flex-col">
              ...
              <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-extrabold text-blue-600">${(event.price_cents / 100).toFixed(2)}</p>
                </div>
                ...
                >
                  {availableTickets > 0 ? 'View Detail' : 'No Seats'}
                </Link>
              </div>
            </div>
          </div>
